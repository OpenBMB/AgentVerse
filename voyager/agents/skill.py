...
class SkillManager:
    def __init__(
        ...
        if resume:
            print(
                f"\033[33m{self.username}: Loading Skill Manager from {ckpt_dir}/skill\033[0m"
            )
    ...
    def add_skill(self, program_name, program_code):
        ...
        print(
            f"\033[33m{self.username}: Skill Manager generated description for {program_name}:\n{skill_description}\033[0m"
        )
        if program_name in self.skills:
            print(
                f"\033[33m{self.username}: Skill {program_name} already exists. Rewriting!\033[0m"
            )
    ...
    def retrieve_skills(self, query):
        ...
        if k == 0:
            return []
        print(
            f"\033[33m{self.username}: Skill Manager retrieving for {k} skills\033[0m"
        )
        docs_and_scores = self.vectordb.similarity_search_with_score(query, k=k)
        print(
            f"\033[33m{self.username}: Skill Manager retrieved skills: "
