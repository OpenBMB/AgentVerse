...
class CriticAgent:
    ...
    def render_human_message(self, *, events, task, context, chest_observation):
        ...
        for i, (event_type, event) in enumerate(events):
            if event_type == "onError":
                print(
                    f"\033[31m{self.username}: Critic Agent: Error occurs {event['onError']}\033[0m"
                )
                return None
        ...
        print(
            f"\033[31m{self.username}: ****Critic Agent human message****\n{observation}\033[0m"
        )
    ...
    def human_check_task_success(self):
        ...
        while not confirmed:
            ...
            print(f"{self.username}: Success: {success}\nCritique: {critique}")
    ...
    def ai_check_task_success(self, messages, max_retries=5):
        if max_retries == 0:
            print(
                "\033[31m{self.username}: Failed to parse Critic Agent response. Consider updating your prompt.\033[0m"
            )
            return (
                False,
                "",
                "Failed to parse Critic Agent response. Consider updating your prompt.",
                messages,
            )

        if messages[1] is None:
            return False, "", "human_message not found. May due to execution error.", ""

        critic = self.llm(messages).content
        print(
            f"\033[31m{self.username}: ****Critic Agent ai message****\n{critic}\033[0m"
        )
        try:
            ...
            return (
                ...
                response,
                messages,
            )  # TODO return reasoning
        except Exception as e:
            print(
                f"\033[31m{self.username}: Error parsing critic response: {e} Trying again!\033[0m"
            )
