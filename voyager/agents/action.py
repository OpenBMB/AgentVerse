...
class ActionAgent:
    def __init__(
        ...
        if resume:
            print(
                f"\033[32m{self.username}: Loading Action Agent from {ckpt_dir}/action\033[0m"
            )
    ...
    def update_chest_memory(self, chests):
        for position, chest in chests.items():
            if position in self.chest_memory:
                ...
                print(
                    f"\033[32m{self.username}: Action Agent saving chest {position}: {chest}\033[0m"
                )
                self.chest_memory[position] = chest
    ...
    def render_human_message(
        ...
        for i, (event_type, event) in enumerate(events):
            if event_type == "onChat":
                ...
                error_messages.append(str(event["onError"]))
