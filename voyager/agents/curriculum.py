...
class CurriculumAgent:
    def __init__(
        ...
        if resume:
            print(
                f"\033[35m{self.username}: Loading Curriculum Agent from {ckpt_dir}/curriculum\033[0m"
            )
    ...
    def render_system_message(self):
        ...
        system_template = load_prompt("curriculum")
        system_message_prompt = SystemMessagePromptTemplate.from_template(
            system_template
        )
        system_message = system_message_prompt.format(
            goal=self.goal, sub_task=self.sub_task
        )
    ...
    def render_human_message(self, *, events, chest_observation):
        ...
        print(
            f"\033[35m{self.username}: ****Curriculum Agent human message****\n{content}\033[0m"
        )
    ...
    async def apropose_next_task(self, *, events, chest_observation, max_retries=5):
        if self.progress == 0 and self.mode == "auto":
            task = "Mine 1 wood log"
            context = "You can mine one of oak, birch, spruce, jungle, acacia, dark oak, or mangrove logs."
            return task, context
        ...
        if self.mode == "auto":
            return await self.apropose_next_ai_task(
    ...
    def propose_next_ai_task(self, *, messages, max_retries=5):
        ...
        print(
            f"\033[31m****{self.username}: Curriculum Agent ai message****\n{curriculum}\033[0m"
        )
        try:
            ...
            print(
                f"\033[35m{self.username}: Error parsing curriculum response: {e}. Trying again!\033[0m"
            )
    ...
    async def apropose_next_ai_task(self, *, messages, max_retries=5):
        curriculum = (await self.llm._call_async(messages)).content
        print(
            f"\033[31m****{self.username}: Curriculum Agent ai message****\n{curriculum}\033[0m"
        )
        try:
            response = self.parse_ai_message(curriculum)
            assert "next_task" in response
            context = await self.aget_task_context(response["next_task"])
            return response["next_task"], context
        except Exception as e:
            print(
                f"\033[35m{self.username}: Error parsing curriculum response: {e}. Trying again!\033[0m"
    ...
    def propose_next_manual_task(self):
        ...
        while not confirmed:
            ...
            print(f"{self.username}: Task: {task}\nContext: {context}")
    ...
    def decompose_task(self, task, events):
        ...
        print(
            f"\033[31m{self.username}: ****Curriculum Agent task decomposition****\nFinal task: {task}\033[0m"
        )
    ...
    async def adecompose_task(self, task, events):
        messages = [
            SystemMessage(
                content=load_prompt("curriculum_task_decomposition"),
            ),
        ...
        response = (await self.llm._call_async(messages)).content
        print(
            f"\033[31m{self.username}: ****Curriculum Agent task decomposition****\n{response}\033[0m"
        )
        return fix_and_parse_json(response)
    ...
    async def arun_qa(self, *, events, chest_observation):
        questions_new, _ = await self.arun_qa_step1_ask_questions(
            events=events, chest_observation=chest_observation
        )
        questions = []
        answers = []
        for question in questions_new:
            if self.qa_cache_questions_vectordb._collection.count() > 0:
                ...
                if docs_and_scores and docs_and_scores[0][1] < 0.05:
                    ...
                    answers.append(answer_cached)
                    continue
            answer = await self.run_qa_step2_ansawer_questions(question=question)
            assert question not in self.qa_cache
            self.qa_cache[question] = answer
            self.qa_cache_questions_vectordb.add_texts(
                texts=[question],
    ...
    async def aget_task_context(self, task):
        # if include ore in question, gpt will try to use tool with skill touch enhancement to mine
        question = (
            f"How to {task.replace('_', ' ').replace(' ore', '').replace(' ores', '').replace('.', '').strip().lower()}"
            f" in Minecraft?"
        ...
        if question in self.qa_cache:
            ...
            answer = await self.arun_qa_step2_answer_questions(question=question)
            self.qa_cache[question] = answer
            self.qa_cache_questions_vectordb.add_texts(
                texts=[question],
            )
    ...
    def run_qa_step1_ask_questions(self, *, events, chest_observation):
        ...
        try:
            ...
            print(
                f"\033[35m{self.username}: Error parsing curriculum response for "
                f"QA step 1 ask questions: {e}.\033[0m"
            )
        return questions, concepts

    async def arun_qa_step1_ask_questions(self, *, events, chest_observation):
        biome = events[-1][1]["status"]["biome"].replace("_", " ")
        questions = [
            f"What are the blocks that I can find in the {biome} in Minecraft?",
            f"What are the items that I can find in the {biome} in Minecraft?",
        ...
        qa_response = (await self.qa_llm(messages)).content
        try:
            # Regex pattern to extract question and concept pairs
            pattern = r"Question \d+: (.+)\nConcept \d+: (.+)"
            # Extracting all question and concept pairs from the text
    ...
    def run_qa_step2_answer_questions(self, question):
        ...
        print(f"\033[35m{self.username}: Curriculum Agent Question: {question}\033[0m")
        qa_answer = self.qa_llm(messages).content
        print(f"\033[31m{self.username}: Curriculum Agent {qa_answer}\033[0m")
        return qa_answer

    async def arun_qa_step2_answer_questions(self, question):
        messages = [
            self.render_system_message_qa_step2_answer_questions(),
            self.render_human_message_qa_step2_answer_questions(question=question),
        ]
        print(f"\033[35m{self.username}: Curriculum Agent Question: {question}\033[0m")
        qa_answer = (await self.qa_llm(messages)).content
        print(f"\033[31m{self.username}: Curriculum Agent {qa_answer}\033[0m")
        return qa_answer
    ...
    async def arun_qa_step2_answer_questions(self, question):
        ...
        print(f"\033[35m{self.username}: Curriculum Agent Question: {question}\033[0m")
        qa_answer = (await self.qa_llm._call_async(messages)).content
        print(f"\033[31m{self.username}: Curriculum Agent {qa_answer}\033[0m")
        return qa_answer
