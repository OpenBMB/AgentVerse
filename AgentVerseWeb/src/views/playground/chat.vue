<template>
  <section class="container-box flex-column">
    <div ref="listRef" class="history-list flex-column">
      <template v-for="({ content, msgID, role, isLatest }, index) in chatMsgInfo" :key="index">
        <TransitionGroup name="message" tag="div">
          <div v-if="role === 'USER'" class="input flex-row">
            ...
            <div class="user-content-border">
              <span> <strong>task_description: </strong>{{ content.task_des }} </span>
              <div class="divider"></div>
              <span> <strong>cnt_agents: </strong>{{ content.cnt_agents }} </span>
            </div>
          </div>

          <div v-else-if="role === 'AI'" class="result typed-box flex-row">
            <!-- <img class="avatar  round-corner-logo" alt="logo" width="52" height="52"
              src="@/assets/images/playground/main-logo-avatar.png" /> -->
            <div class="content">
              <Tab :ref="tabchildList" :conversationId="conversationId" :key="msgID" :mode="remSetting?.mode || 'auto'"
                @disconnect="disConnectWebsocket" @runSubtask="RunNextSubtask" @runInner="RunNextinnerNode"
...
import TaskRefineInfo from './components/TaskRefineInfo.vue'
...
//   chatMsgInfoStore.setCurrentInput({ task_des: 'Record Playing', cnt_agents: '3' });
// }
...
// roundStore.setAutoMode(remSetting?.mode === 'auto');
...
const handleEvaluation = () => {
  ...
  ElMessage({
    type: 'error',
    message: 'Your task needs to be further resolved.'
  })
};
...
const plainUrl = `${protocol}//${BACKEND_URL.replace('https://', '').replace('http://', '')
  }/ws/${conversationId.value}?user_id=${user_id}&token=${token}&description=${currentInput.value.task_des.substring(0, 26)}`;

// const playbackUrl = `${protocol}//${BACKEND_URL.replace('https://', '').replace('http://', '')
//   }/ws_replay/${conversationId.value
...
const wsMessageHandler = (data: any) => {
  ...
  switch (status) {
    case 'start':
      console.log("!!! start wsMessageHandler: ", data)
      roundStore.initializeRounds(data.data);
      console.log("!!! start wsMessageHandler subround: ", roundStore.subrounds)
      break;

    case 'stage':
      console.log("!!! stage wsMessageHandler: ", data)
      console.log("!!! stage this subrounds recruitment: ", roundStore.subrounds[roundStore.current_subround_index].recruitment)
      console.log("!!! stage this current_index: ", roundStore.current_subround_index)
      roundStore.addPipeline(data.data);
      console.log("!!! stage this subrounds ds: ", roundStore.subrounds[roundStore.current_subround_index].decision_make)
      console.log("!!! stage this subrounds action: ", roundStore.subrounds[roundStore.current_subround_index].aciton_exectuion)
      console.log("!!! stage this subrounds evaluation: ", roundStore.subrounds[roundStore.current_subround_index].evaluation)
      break;
    case 'subround':
      console.log("!!! subround wsMessageHandler: ", data)
      roundStore.addSubround(data.data)
      break;

    case 'refinement':
      console.log("!!! refinement wsMessageHandler: ", data)
      roundStore.addRefinementInfo(data.data)
      handleEvaluation()
      break;

    case 'finished':
      console.log("!!! finished wsMessageHandler: ", data)
      roundStore.addRefinementInfo(data.data)
      handleFinish();
      disConnectWebsocket();
      break;
...
// 输入回答
const sendMessage = async (val: { task_des: string, cnt_agents: string }) => {
  currentInput.value.task_des = val.task_des
  currentInput.value.cnt_agents = val.cnt_agents
  chatMsgInfo.value = [
    {
      content: val,
      msgID: nanoid(),
...
const RunConnection = () => {
  ...
  switch (pageMode.value) {
    ...
    case 'new':
      ...
      console.log("get current input in chat.vue", input);
      if (input) {
        newTalkConnection();
        sendMessage(input)
      } else {
  ...
  (newVal, oldVal) => {
    ...
    if (pageMode.value === 'playback' || pageMode.value === 'review' || pageMode.value === 'record') {
      ...
      if (pageMode.value === 'playback') {
        ...
        roundStore.setAutoMode(false);
        roundStore.completeSubtask();
      }
...
const queryChatInfo = async (id: string) => {
  ...
  currentInput.value.task_des = useMessage.content
