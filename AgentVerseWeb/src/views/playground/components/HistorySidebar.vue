<template>
  <section class="sidebar">
    <div class="logo_name_border flex-column">
      <img class="logo" alt="" draggable="false" src="@/assets/images/playground/agentverse-main-logo.png" width="144"
        height="120" />
      <img class="name" src="@/assets/images/playground/agentverse-name-logo.png" draggable="false" alt="" width="180"
        height="35" />
    </div>
    <div class="new-talk flex-row flex-center" @click="createNewTalk" v-if="false">
      <span>New Talk</span>
    </div>
    <div class="history-list flex-column" v-if="false">
      <div v-for="(item, index) in historyTalkArr" :key="item.interaction_id" class="history-item flex-row"
        :class="{ active: item.interaction_id === route.query.id }">
        <img src="@/assets/images/playground/qp.svg" alt="" class="icon" />
        <span class="ellipsis" @click="switchToConversation(item)">
          {{ item.description }}
        </span>

        <span class="delete_btn flex-row">
          <span class="playback-icon" title="Run the task" @click="playbackItem(item, index)">
            <VideoPlay />
          </span>
          <el-popconfirm width="fit-content" popper-class="delete-tip" confirm-button-text="Confirm"
            cancel-button-text="Cancel" :hide-icon="true" :hide-after="0" placement="top"
      ...
      <div v-if="loading" class="no-data loading flex-column flex-center" data-loading="Data Loading..."></div>
    </div>
    <div class="clear-btn-border flex-center">
      <el-dropdown placement="top" trigger="click" class="clear-btn flex-row flex-center" @command="handleCommand">
