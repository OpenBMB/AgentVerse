<template>
  <section class="feedback flex-row flex-center">
    <i class="copy-icon flex-row flex-center" title="复制" @click="copy">
      <IconCopy />
    </i>
    <i class="flex-row flex-center" title="赞" :class="{ active: data.rating === 'THUMBS_UP' }"
      @click="evaluate('THUMBS_UP')">
      <IconGood />
    </i>
    <el-popover ref="feedbackRef" placement="bottom-end" trigger="click" popper-class="bad-tip-popover" width="100%"
      @blur="isShowFeedback = false">
      <template #reference>
        <i class="flex-row flex-center" title="踩" :class="{ active: data.rating === 'THUMBS_NO' && data.feedbackMsg }"
      ...
      <div class="bad-tip flex-column">
        <div class="title flex-row">
          <span>💡您的反馈将帮助AgentVerse优化进步</span>
        </div>
        <div v-for="({ title, tip }, index) in tipList" :key="index">
          <div class="tip-item-title">{{ title }}</div>
          <div class="tip-item flex-row">
            <span v-for="(item, idx) in tip" :key="idx" class="content"
              :class="{ active: feedbackForm.activeOptions.includes(item) }" @click="changeTag(item)">
              {{ item }}
            </span>
          </div>
        </div>
