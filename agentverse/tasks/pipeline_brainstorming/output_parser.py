...
@output_parser_registry.register("pipeline_brainstorming")
class PipelineBrainstormingParser(OutputParser):
...
class RoleAssignerParser(OutputParser):
    ...
    def parse(self, output: LLMResult) -> List[str]:
        ...
        if len(roles) < self.cnt_critic_agents:
            logger.error(f"Role assigner failed to assign roles to {self.cnt_critic_agents} critics!")
...
class EvaluatorParser(OutputParser):
    ...
    def parse(self, output: LLMResult) -> Tuple[List[int], str]:
        ...
        try:
            ...
            score = [int(pattern.findall(checks[i])[0])
