...
class Workspace:
    ...
    def _sanitize_path(
        ...
        print(f"Resolving path '{relative_path}' in workspace '{root}'")

        root, relative_path = Path(root).resolve(), Path(relative_path)

        print(f"Resolved root as '{root}'")
        ...
        print(f"Joined paths as '{full_path}'")
...
class WorkSpaceDispatcher:
    """不同的节点对应的工作区状态可能是相同的，由于我们的搜索树节点生成后就不会再变化，他们可以映射到同一个虚拟工作区文件夹
    """
    def __init__(self):
        pass