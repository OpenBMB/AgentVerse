...
class AgentVersePipeline:
    ...
    def from_task(cls, task: str):
        ...
        env_config["critics"] = [load_agent(task_config["agents"][2])
                                 for i in
                                 range(task_config["cnt_critic_agents"])]
        env_config["executor"] = load_agent(task_config["agents"][3])
        env_config["evaluator"] = load_agent(task_config["agents"][4])
        env_config["cnt_critic_agents"] = task_config.get("cnt_critic_agents",
                                                          0)
        ...
        agents = [environment.role_assigner, environment.solver] + \
                 environment.critics + [environment.evaluator]
        for agent in agents:
            agent.environment = environment

        return cls(agents, environment)

    def run(self, single_agent: bool = False,
        ...
        **Rewrite in pipeline to let the pipeline architecture more clear**
        ...
        criticisms: List[AgentCriticism] = []
        while self.environment.cnt_round < self.environment.max_loop_rounds:
            ...
            if not single_agent:
                advice += "\n" + "".join(x.criticism for x in criticisms)
                preliminary_solution, new_criticisms = self.multiagent_criticizing(
                    preliminary_solution, advice, discussion_mode)
                criticisms += new_criticisms
            else:
                # single agent
                ...
                for i in range(self.environment.max_criticizing_rounds):
                    ...
                    '''
                    for j in range(self.environment.cnt_critic_agents):
                    ...
                    # summarized = self.environment.summarize(
                    #     content='\n'.join(solutions_in_this_round),
                    # )
                    # logger.info(f"New Summarized:\n{summarized}")
                    # preliminary_solution += '\n' + summarized
                    preliminary_solution += '\n' + '\n'.join(solutions_in_this_round)
            ...
            if score is None or any([s < 8 for s in score]):
    ...
    def multiagent_criticizing(self, preliminary_solution: str,
                               advice: str = "",
                               discussion_mode: bool = False) -> \
        ...
        for i in range(self.environment.max_criticizing_rounds):
            # logger.info(f"Round {i} of criticizing")
            criticisms = asyncio.run(
                self.environment.criticize(preliminary_solution, advice))
            printed_message = '\n'.join(
            ...
            if self.is_consensus_reached(criticisms):
                ...
                break
            criticism = [(criticism.sender_agent, criticism.criticism)
                         for criticism in criticisms
                         if not criticism.is_agree]
            if discussion_mode:
                preliminary_solution = self.environment.summarize(preliminary_solution)
            else:
                preliminary_solution = self.environment.solve(
                    former_solution=preliminary_solution,
                    critic_opinions=criticism,
                )
            logger.info(f"New Solution at round{i} of criticizing:\n{preliminary_solution}")

        final_solution = preliminary_solution
        return final_solution, criticisms
    ...
    def save_result(self, result: str, single_agent: bool = False):
        ...
        if single_agent:
            result_file_path = "../results/" + \
                               self.environment.task + "_single.txt"
        else:
